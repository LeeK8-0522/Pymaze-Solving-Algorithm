...
class Visualizer(object):
    ...
    def show_maze(self):
        ...
        fig = self.configure_plot()  # figure 관련 인스턴스 생성 및 설정.

        # Plot the walls on the figure
        self.plot_walls()  # 미로의 wall 그리기
    ...
    def plot_walls(self): # matplot library에 내장된 text, plot 함수를 이용하여 미로의 wall 그리기
        """ Plots the walls of a maze. This is used when generating the maze image"""
        for i in range(self.maze.num_rows): # cf) plot(x, y, color) -> (x, y) 순서쌍으로 이루어진 선을 color 색깔로 긋기 (color 대신 marker로 설정 가능 O)
    ...
    def configure_plot(self):  # figure, axes 인스턴스 생성. text 함수를 통해 좌표 글자로 출력.
        """Sets the initial properties of the maze plot. Also creates the plot and axes"""

        # Create the plot figure
        fig = plt.figure(figsize = (7, 7*self.maze.num_rows/self.maze.num_cols)) # 너비를 7로 고정하고 비율에 맞게 figure을 설정

        # Create the axes
        self.ax = plt.axes() # ax는 fig에 포함 O
        # cf) figure -> like 액자, axes -> like 액자 속의 도화지 (실제 그림을 그릴 공간)

        # Set an equal aspect ratio
        self.ax.set_aspect("equal") # 가로 세로 비율이 1 : 1이 되도록 axes를 설정
    ...
    def show_maze_solution(self):
        ...
        list_of_backtrackers = [path_element[0] for path_element in self.maze.solution_path if path_element[1]]  # 활성 상태인 path_element들을 저장 (using 리스트 컴프리헨션)

        # Keeps track of how many circles have been drawn
        circle_num = 0  # 그린 cell(혹은 circle)의 개수를 저장
    ...
    def show_generation_animation(self):  # 미로 생성 과정을 애니메이션으로 보여주는 메서드. (과제에서는 불필요)
